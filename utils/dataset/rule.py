...
class ECRuleDataset(Dataset):
    def __init__(self, data_root, vocab_root, train=True):
        ...
        self.rule_labels = pickle.load(open('/data/wujipeng/ec/data/raw_data/rule_labels.pkl', 'rb'))
        self.read_data()
        self.read_vocab()
    ...
    def collate_fn(self, batch_data, pad=True, clause_length=41, batch_size=16):
        ...
        return ids, np.array(clauses), natures, np.array(keywords), np.array(emotions), np.array(poses), np.array(
            labels), np.array(rule_labels)
